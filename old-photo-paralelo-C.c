...
{
    int read_fd = *(int*)arg;
    char image_ref[MAX_IMAGE_REF_LENGTH];

    while (read(read_fd, image_ref, sizeof(image_ref)) > 0){
        if (strcmp(image_ref, "") == 0) { // Verifica se é o sinalizador de finalização
            break;
        }
    }
        ...
        //filename da imagem criada a ser guardada no disk.
        ...
        if (in_texture_img == NULL) {
            fprintf(stderr, "Erro ao ler a imagem de textura.\n");
            return NULL;
        }
        ...
        {
            if(!strlen(imagens_thread[i])) continue;    // Verifica se o nome da imagem está vazio
...
int main(int argc, char *argv[])
{
    ...
    // Envio de referências de imagens pelo pipe
    for (int i = 0; i < count; i++) {
        // Substitua com a lógica correta para obter o caminho da imagem
        write(fd[1], imagens_thread[i], strlen(imagens_thread[i]) + 1);
    }
    write(fd[1], "", 1); // Sinalizador de finalização
